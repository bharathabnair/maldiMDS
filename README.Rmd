...
maldiMDS calculates MALDI Deamidation Score (MDS) from MALDI-TOF ZooMS data.
It is a sample level measure of the glutamine deamidation, aggregated from the
extent of deamidation of a list of peptides.

It is the next iteration and based on the MALDIpqi package (https://github.com/ismaRP/MALDIpqi.git).
maldiMDS is thought to be more general and flexible, not only applicable to parchment data, but also
bone faunal assemblages. To reflect this, we are changing the name, but we're still keeping MALDIpqi in a separate repository.

For details in the data processing and mathematical method and models to estimate PQI, refer to our publication Nair et al. (2022). The method was initially developed to estimate parchment quality based on deamidation, following the ideas from Wilson et al. (2012). However, it can be applied in other tissues as in van Doorn et al. (2012) or Brown et al. (2021)
